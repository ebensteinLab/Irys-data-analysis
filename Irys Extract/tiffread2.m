...
stack=zeros(512,512,max(img_last)-min(img_first)+1);
...
while (ifd_pos ~= 0)
    ...
    if image_counter<min(img_first)
        file_pos  = ftell(TIF.file);
        fseek(TIF.file, file_pos+12*num_entries, -1);
        ifd_pos = fread(TIF.file, 1, 'uint32', TIF.BOS);
        img_skip = img_skip + 1;
        continue;
    end
    %{
    if image_counter>max(img_last)
        ifd_pos=0;        
        continue;
    end
    %}
    ...
    if isfield( TIF, 'MM_stack' )
        ...
        if ( img_skip + img_read >= max(img_last) )
            break;
        end
    end
end
